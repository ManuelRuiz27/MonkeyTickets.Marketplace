...
export class TicketsService {
    ...
    async checkInTicket(qrCode: string, session: ActiveStaffSession) {
        ...
        const [,] = await this.prisma.$transaction(updates);

        // V2: If generated by RP, increment counter separately to avoid typing issues
        if (ticket.rpProfileId) {
            await this.prisma.rPProfile.update({
                where: { id: ticket.rpProfileId },
                data: {
        ...
        // Re-fetch ticket with all relations for response
        const finalTicket = await this.prisma.ticket.findUnique({
            where: { id: ticket.id },
            include: {
                order: {
                    include: {
        ...
        return {
            ...
            ticket: {
                id: finalTicket!.id,
                qrCode: finalTicket!.qrCode,
                status: 'USED' as const,
                usedAt: finalTicket!.usedAt!,
                buyer: finalTicket!.order.buyer,
                template: finalTicket!.template,
            },
            event: {
                id: ticket.order.event.id,
                title: finalTicket!.order.event.title,
                attendanceCount: ticket.order.event.attendanceCount + 1, // Calculated locally
            },
            rpInfo: ticket.rpProfile ? {
                rpName: ticket.rpProfile.user.name,
                guestType: ticket.guestType?.name || null,
                guestNickname: ticket.guestNickname || null,
                guestTypeColor: ticket.guestType?.color || null,
                guestTypeIcon: ticket.guestType?.icon || null,
            } : null,
        };
    }
